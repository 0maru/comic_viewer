...
///
/// By default, InteractiveView may draw outside of its original area of the
...
/// InteractiveView from drawing outside of its original size, wrap it in a
/// [ClipRect]. Or, to prevent dead areas where InteractiveView does not
/// receive gestures, be sure that the InteractiveView widget is the size of
/// the area that should be interactive. See
/// [flutter-go](https://github.com/justinmc/flutter-go) for an example of
/// robust positioning of an InteractiveView child that works for all screen
...
///     which includes the use of InteractiveView.
///
...
class InteractiveView extends StatefulWidget {
  ...
  /// is often useful when a child should be bigger than the InteractiveView.
  ///
  /// Defaults to true.
  ///
  /// {@tool dartpad --template=stateless_widget_scaffold}
  ...
  ///
  /// {@template flutter.widgets.InteractiveView.onInteraction}
  /// Will be called even if the interaction is disabled with
  /// [panEnabled] or [scaleEnabled].
  ///
  /// A [GestureDetector] wrapping the InteractiveView will not respond to
  ...
  ///
  /// {@macro flutter.widgets.InteractiveView.onInteraction}
  ///
  /// See also:
  ///
  ///  * [onInteractionUpdate], which handles an update to the same interaction.
  ...
  /// listeners are notified. If the value is set, InteractiveView will update
  ...
  static Vector3 getNearestPointInside(Vector3 point, Quad quad) {
    ...
    final List<Vector3> closestPoints = <Vector3>[
      InteractiveView.getNearestPointOnLine(point, quad.point0, quad.point1),
      InteractiveView.getNearestPointOnLine(point, quad.point1, quad.point2),
      InteractiveView.getNearestPointOnLine(point, quad.point2, quad.point3),
      InteractiveView.getNearestPointOnLine(point, quad.point3, quad.point0),
    ];
...
class InteractiveViewState extends State<InteractiveView> with TickerProviderStateMixin {
  ...
  double _currentRotation = 0.0; // Rotation of _transformController.value.
  ...
  void _onScaleUpdate(ScaleUpdateDetails details) {
    ...
    switch (_gestureType) {
      case _GestureType.scale:
        ...
        final Offset focalPointSceneCheck = _transformController.toScene(
          details.localFocalPoint,
        );
      ...
      case _GestureType.rotate:
        ...
        _transformController.value = _matrixRotate(
          _transformController.value,
  ...
  void _onScaleEnd(ScaleEndDetails details) {
    ...
    final Vector3 translationVector = _transformController.value.getTranslation();
  ...
  void _onAnimate() {
    ...
    final Vector3 translationVector = _transformController.value.getTranslation();
    final Offset translation = Offset(translationVector.x, translationVector.y);
    final Offset translationScene = _transformController.toScene(
      translation,
    );
    final Offset animationScene = _transformController.toScene(
      _animation.value,
    );
  ...
  void initState() {
    ...
    _transformController = widget.transformationController ?? TransformController();
    _transformController.addListener(_onTransformationControllerChange);
  ...
  @override
  void didUpdateWidget(InteractiveView oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Handle all cases of needing to dispose and initialize
    // transformationControllers.
    if (oldWidget.transformationController == null) {
      if (widget.transformationController != null) {
        _transformController.removeListener(_onTransformationControllerChange);
...
/// See also:
///
///  * [InteractiveView.transformationController] for detailed documentation
///    on how to use TransformationController with [InteractiveView].
class TransformationController extends ValueNotifier<Matrix4> {
  ...
  ///       _childWasTappedAt = _transformController.toScene(
...
Offset _exceedsBy(Quad boundary, Quad viewport) {
  ...
  for (final Vector3 point in viewportPoints) {
    final Vector3 pointInside = InteractiveView.getNearestPointInside(point, boundary);
